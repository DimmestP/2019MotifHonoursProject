...
coefs <- coef(motifs_cv_glm, s = "lambda.min")
#Create a tibble of coefficients ordered by absolute value
sig_glms <- tibble(
  motif_glmnet = rownames(data.matrix(coefs))[data.matrix(coefs) != 0],
  coeff_glmnet = data.matrix(coefs)[data.matrix(coefs) != 0]) %>%
